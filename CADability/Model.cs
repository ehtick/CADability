...
namespace CADability
{
            ...
            IEnumerable, ICommandHandler, ICategorizedDislayLists, IDeserializationCallback, IJsonSerialize, IJsonSerializeDone
    {
        ...
        {   // wird aus dem ThreadPool heraus gestartet. 
            ...
            IGeoObject go = state as IGeoObject; // muss klappen
            lock (runningThreads)
            {
                if (runningThreads.ContainsKey(go.UniqueId))
                {
                    runningThreads[go.UniqueId] = Thread.CurrentThread;
                }
                else return; // es wurde abgebrochen, nichts zu tun, man kann leider nicht aus der Queue entfernen
            }
            try
            {   // hier wird die eigentliche Arbeit gemacht
                go.PrepareDisplayList(backgroundRecalcPrecision);
                //if (go is Face) System.Diagnostics.Trace.WriteLine("Recalc Face done: " + go.ToString());
            }
            catch (ThreadAbortException)
            {   // z.Z. wird nicht abgebrochen, könnte aber noch implementiert werden
            }
            finally
            {
                lock (runningThreads)
                {
                    runningThreads.Remove(go.UniqueId); // wirft keine Exception wenn nicht drin
        ...
        {   // In die Queue von ThreadPool.QueueUserWorkItem einfügen.
            ...
            if (toInsert is Solid)
            {
                Solid s = toInsert as Solid;
                for (int i = 0; i < s.Shells.Length; ++i)
                {
                    InsertRecalcGeoObject(s.Shells[i]);
                }
            }
            else if (toInsert is Shell)
            {
                Shell s = toInsert as Shell;
            ...
            {
                lock (runningThreads)
                {   // Hier werden die Queue gefüllt und gleichzeitig runningThreads aktualisiert
                    // if (runningThreads.ContainsKey(toInsert.UniqueId))
                    // was soll die blöde Abfrage, es wird ja sonst nirgens gefüllt
                    {
                        runningThreads[toInsert.UniqueId] = null;
                    }
                    ThreadPool.QueueUserWorkItem(new WaitCallback(RecalcGeoObject), toInsert);
                }
            }
        }
        internal void DoBackgroundRecalcDisplayList(object precisionParameter)
        {   // wird von RecalcDisplayLists  in einem neuen Thread aufgerufen.
            ...
            double precision = (double)precisionParameter;
            if (runningThreads == null) runningThreads = new Dictionary<int, Thread>();
            try
            {
                backgroundRecalcPrecision = precision;
                ...
                {   // Beim zufügen und entfernen muss diese Aktion abgebrochen werden, damit man nicht über den Index hinausgeht
                    ...
                    InsertRecalcGeoObject(geoObjects[i]);
                }
                ...
                displayListPrecision = backgroundRecalcPrecision;
            ...
            {   // es wurde abgebrochen, weil eine noch genauere Zoomstufe benötigt wird
                lock (runningThreads)
                {
                    // folgendes klemmt manchmal:
                    //foreach (Thread t in runningThreads.Values)
                    //{
                    //    t.Abort(); // die laufen gerade und werden abgebrochen
                    //    // die noch in der Warteschlange sind verabschieden sich sofort wenn sie starten
                    //    // naja, das stimmt nicht genau, evtl. wurde schon DoBackgroundRecalcDisplayList
                    //    // erneut aufgerufen und dann sind sie wieder in der Liste und werden somit
                    //    // worst case zweimal ausgeführt. Allerdings tun die Objekte beim zweitenmal
                    //    // praktisch nichts, da precision schon stimmt
                    //}
            ...
            if (paintTo3D.Precision < displayListPrecision && !paintTo3D.DontRecalcTriangulation)
            {
                ...
                {   // abbrechen und warten
                    ...
                    if (t != null)
                    {
                        ...
                        t.Abort(); // manageBackgroundRecalc kann hier drin null werden
                        t.Join();
                    }
                }
                manageBackgroundRecalc = new Thread(new ParameterizedThreadStart(DoBackgroundRecalcDisplayList));
                manageBackgroundRecalc.Start(paintTo3D.Precision / 2.0); // /2.0, damit es nicht sooft drankommt
